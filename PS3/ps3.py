...
def is_valid_word(word, hand, wordlist):
    ...
    if word.find('*') == -1:
        ...
        for letter in VOWELS:
            if word[0:ind] + letter + word[(ind+1):] in wordlist:
                return True
        return False    
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while calculate_handlen(hand) > 0:
        ...
        # Otherwise (the input is not two exclamation points):
            ...
            if is_valid_word(inputWord, hand, wordlist):

                # Tell the user how many points the word earned,
                # and the updated total score
                score += get_word_score(inputWord, HAND_SIZE)
                print(inputWord+' earned '+ str(get_word_score(inputWord, HAND_SIZE))+' points. Total: '+str(score)+' points.')
...
def substitute_hand(hand, letter):
    ...
    c = CONSONANTS+VOWELS
    d = ""
    for temp in hand.keys():
        d += temp   
    x = random.choice(c.strip(d))
    hand[x] = hand[letter]
    del(hand[letter])
